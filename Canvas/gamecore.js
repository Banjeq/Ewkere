...
async function updateGame() {
	...
	if(game.state == "playing" && sss == false && gameTime == 0){
		...
		hudOverlay.gameScore = "score: "+score
  ...
  if (!pause){	
	...
	if(player.state == "hit" && health == 0) {
			...
			if(player.counter >= 40) {
				...
				overlay.counter = 100;
				overlay.controls5 = "time played: "+timePlayed
				overlay.controls6 = "score: "+ score
  ...
  if (!pause){  
	if(game.state == "newGame") {
		newOverlay.counter = 100;
			player.state = "dead"
			newOverlay.title = "Space N Vaders"
	...
	if(game.state == "newGame1") {
			...
			player.state = "dead"
			newOverlay.title = "Space N Vaders"
			newOverlay.controls1 = "choose spaceship:"
